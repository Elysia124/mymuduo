...
void Thread::start()
{
    ...
    if (sem_init(&sem, 0, 0) != 0) {
        LOG_FATAL << "sem_init failed: errno=" << errno << " error=" << strerror(errno);
    }
    ...
    thread_ = std::thread([tid = &tid_, &sem, func = std::move(func_)]() mutable {
        *tid = CurrentThread::tid();
        ...
        if (sem_post(&sem) != 0) {   // +1
            LOG_FATAL << "sem_post failed: errno=" << errno << " error=" << strerror(errno);
        }
    ...
    while (sem_wait(&sem) != 0) {   // -1
        if (errno != EINTR) {
            LOG_FATAL << "sem_wait failed: errno=" << errno << " error=" << strerror(errno);
        }
    }
